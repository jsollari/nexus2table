...
int main(int argc, char *argv[]){
	...
	if(argc != 3)
		printerr("needs .nex file, output filename (no extension)");

	inp = fopen(argv[1],"r");
	if(inp == NULL)
		printerr("cannot open .nex file");
	...
	if(outp == NULL)
		printerr("cannot create output file");
	...
	while(!reachEOF){
		...
		if(!reachEOF){
			...
			if(strcmp(aux,"begin")==0||strcmp(aux,"BEGIN")==0||strcmp(aux,"Begin")==0){
				...
				if(strcmp(aux,"data;")==0||strcmp(aux,"DATA;")==0||strcmp(aux,"Data;")==0){
					...
					i=0; j=0;
				   	...
				   	if(!(strcmp(info1,"DNA")==0||strcmp(info1,"dna")==0||strcmp(info1,"Dna")==0))
				   		printerr("program can't deal with specified data");
						...
						if(c1 == '[')
							...
							while(csite<nsites){
								if((c1=getc(inp))=='T'||c1=='A'||c1=='C'||c1=='G'||
									...
									printerr("problem reading MATRIX");
							}
							csamp++;
						}
					}
							...
							while(!endPop){
								...
								while(i<strlen(aux)){
									if(aux[i]==';'){
										...
										printerr("problem reading TAXSET");
								}
								if(firstInd)
									id[nr1-1]=cpop;
								else{
									if(nr1>=nr2)
										printerr("problem defining TAXSET: n1>=n2");
									for(i=nr1;i<=nr2;i++)
										id[i-1]=cpop;
								}
							}
				   			...
				   			printerr("unrecognized specifier after BEGIN SETS (use TAXSET)");
				   	}
				   	npop=cpop;
				}
				else
					printerr("unrecognized specifier after BEGIN (use DATA or SETS)");
			}
			else if(aux[0]=='['){
				...
				printerr("unrecognized command (only BEGIN accepted)");
		}
	}
